...
export class PreguntasPage implements OnInit {
  ...
  upload_publication(){
   ...
   // var dt = moment(timei).format('DD-MM-YYYY'); 
    ...
    var id = 9999999999 - this.numeros.length;
    console.log("id: " + id)
    //var id = Math.floor(Math.random() * 3213546846468435454) + 1
    console.log("random",id)
    var sid = id.toString()
    this.fbs.insertar("Proyectos/"+this.proyecto+"/preguntas/", sid, numero )
  ...
  get_comunicados(){
    this.fbs.consultar("/Proyectos/"+this.proyecto+"/preguntas").subscribe((servicios) => {
      this.numeros = [];
  ...
  delete(comunicado){
    ...
    this.fbs.delete_doc("Proyectos/"+this.proyecto+"/preguntas", comunicado).then(() => {
    })
   }
}
