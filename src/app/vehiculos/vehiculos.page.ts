...
export class VehiculosPage implements OnInit {
 ...
 async addTodo(imageurl,id){
  ...
  this.fbs.insertar("Proyectos/"+this.proyecto+"/vehiculos/", id, mascot )
      $('#nombre').val("");
      $('#apto').val("");
      $('#especie').val("");
      $('#raza').val("");
  ...
  async uploadFile(id, file): Promise<any> {
    if(file && file.length) {
      try {
        const task = await this.storage.ref(this.proyecto+'/vehiculos').child(id).put(file[0])
        $('#name').val("");
        return this.storage.ref(this.proyecto+`/vehiculos/${id}`).getDownloadURL().toPromise();
      } catch (error) {
        console.log(error);
      }
    }
  ...
  if(res){
   console.log(item);
    ...
    if(item.imageUrl) {
      this.storage.ref(this.proyecto+`/vehiculos/${item.id}`).delete()
    }
  ...
  ngOnInit() {
    ...
    this.itemsRef = this.db.collection('Proyectos/'+this.proyecto+'/vehiculos')
    this.itemsRefUser = this.db.collection('user/'+this.uid+'/proyectos/'+this.proyecto+'/vehiculos')
